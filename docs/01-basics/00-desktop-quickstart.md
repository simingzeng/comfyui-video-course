...
### 💡 极简指引：如果画布是纯黑空的，怎么调出节点？
由于前面我们选择了“跳过并安装”（为了节省硬盘空间），所以软件打开后是一个**完全空白的画布**。

请跟着以下步骤，**一键调出默认的 5 大核心节点**：

1. 看屏幕左上角的菜单栏，点击 **`图形 ∨`**（或者界面左上角的菜单按钮）。
2. 在下拉菜单中，选择 **`加载默认` (Load Default)**。
3. 鼠标点击后，画布上就会**瞬间弹出一整套互相连好线的彩色基础节点**（包含画风模型、提示词框、采样器和保存图像框）！
