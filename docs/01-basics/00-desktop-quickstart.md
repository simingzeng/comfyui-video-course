...
##### 💡 告别路径重名困扰：如何将实例改名（如改为 `Main5080`）？
如果默认的 `ComfyUI` 名字让你感到困惑，可以在 **Comfy Desktop 启动大厅** 中进行重命名：
1. 点击界面左上角 **`C`** 标志返回启动大厅。
2. 在已安装的实例卡片右上角点击 **`...` (三点菜单)**。
3. 选择 **`重命名` (Rename)**，将名称修改为 `Main5080`（或 `主渲染机`）。
4. 命名修改后，大厅卡片与后续实例标识就会变得极其清晰，再也不会和内核代码目录搞混！
