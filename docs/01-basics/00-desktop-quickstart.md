...
##### 💡 告别路径重名困扰：如何将实例改名（推荐改为 `Main-RTX5080`）？
在 **Comfy Desktop 启动大厅** 的【实例详情】面板中可以一键重命名：
1. **点击右下角按钮**：在实例卡片详情页右下角，点击蓝色【启动】按钮旁边的灰框按钮 **`更多 ∧`**。
2. **点击重命名**：在弹出的菜单中，选择第一项 **`✏️ 重命名`**。
3. **推荐修改名称**：输入 **`Main-RTX5080`**（或 `MainEnv`）。
   * 改名后，硬盘路径会立刻变成极其漂亮的：`ComfyUI-Installs\Main-RTX5080\ComfyUI\`，彻底告别重名困扰！
