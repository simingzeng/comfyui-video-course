...
## 🖼️ AI 出图的 5 步全景数据流与角色分工

别被密密麻麻的连线吓到了，其实 ComfyUI 生成一张图片，**本质上就是一个由 5 位剧组专家各司其职的流水线（从左向右流动）**：

![AI绘图5步极简流程全景图](./5-steps-guide.png)

```text
【1.大画家的脑海】 ➔ UNet 加载器 (扩散模型主脑，决定顶级写实质感)
【2.需求翻译官】   ➔ 加载CLIP + CLIP文本编码 (阿里 Qwen 3.4B 语言大脑，把中文翻译成画家指令)
【3.空白画布】     ➔ 空Latent图像 (设定画框尺寸，如 21:9 宽屏)
【4.画笔作画中枢】 ➔ 采样算法 AuraFlow + K 采样器 (核心画笔，在画布上极速擦除噪点成画)
【5.暗房显影装裱】 ➔ 加载VAE + VAE解码 + 保存图像 (将数学潜空间解密为高清真彩 PNG 并存盘)
```

::: details 💡 为什么画布左侧的【Step1 蓝框】会放了 3 个加载器？
在真机画布上，你会看到左侧有一个叫 `Step1 - Load models` 的蓝色大框，里面整齐摆放着 `UNet加载器`、`加载CLIP` 和 `加载VAE`。  
这是因为在**工程实现**上，这三个节点都负责“从硬盘读取大文件到显存”；但在**业务分工**上，它们分别效力于【大脑】、【翻译官】和【显影师】三大不同角色！
:::

对照下方真机全景标注图，我们逐一进入 5 大剧组角色的内部面板深入拆解：

![ComfyUI 5大核心节点真机位置全景标注图](./01-5-nodes-annotated.png)

---

## 1️⃣ 角色一：大画家的脑海（Diffusion Model / 扩散主脑）
...
* **输出端口**：从右侧引出 **紫色 `MODEL` 连线**，一路连入作画中枢！
...
负责将人类的文字意图，转化为扩散画家听得懂的数学条件指令：

```text
【加载CLIP (Qwen 3.4B)】 ──(黄色CLIP线)──> 【CLIP 文本编码 (输入中文词)】 ──(黄色条件线)──> 【K 采样器: 正面条件】
                                          【CLIP 文本编码 (输入中文词)】 ──(黄色条件线)──> 【条件零化】 ──(清零负向)──> 【K 采样器: 负面条件】
```
...
## 3️⃣ 角色三：画纸与画布（Empty Latent Image 画框规格）

在画布左下角 `Step2 - Image size` 区域，负责设定画框的物理尺寸与并发生成策略：
...
## 4️⃣ 角色四：画笔作画中枢（KSampler 采样器去噪核心）
...
### 1. 物理节点 A：`采样算法 (AuraFlow)`（前置算法调校配件）
...
## 5️⃣ 角色五：暗房显影师与装裱相框（VAE 解码与成片落地）
