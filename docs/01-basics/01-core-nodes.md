...
* **💡 `移位 (Shift 3.00)` 的大白话底层原理**：
  * **画家的两阶段作画规律**：AI 画画可以分为两个阶段：**【前半段：画大轮廓骨架】** 与 **【后半段：雕琢毛发/光影/材质细节】**。
  * **为什么极速模型需要移位？**：普通传统模型有 30 步，时间充裕；而 Z-Image-Turbo 极速模型一共只有 **8 步**！如果时间均匀分配，很容易导致“前几步大轮廓还没画扎实，就匆匆进入后半段”，最终画面结构发虚崩坏。
  * **`Shift 3.00` 的作用**：就像一个**“精力前置分配器”**，它强制模型把更多的计算算力集中在**前 1~3 步光速锁定机械猫的铠甲主体骨架**，从而让后半段步数可以全力打磨 8K 级金属反光与雨夜光影！

---

### 2. 物理节点 B：`K 采样器 (KSampler)` 5 大核心参数透视
...
2. **`步数 (steps)`：主画手的雕琢轮数（为什么不是越大越好？）**：
   * **极速模型的物理极值**：很多新手误以为“步数调到 50 或 100 画质会更好”。其实对于经过极速蒸馏提纯的模型（Z-Image-Turbo / FLUX Schnell），它的数学最优收敛解在第 **8 步**（或 4 步）就已经 100% 达到了画质顶峰。
   * **盲目调大的危害**：如果强行调到 50 步以上，不仅耗时翻 6 倍，还会导致画面出现**“过度锐化、噪点反噬、塑料感过重、五官局部崩坏”**！
...
::: details 🖌️ 采样器 (Sampler) 4 大主流流派大拆解（图 1 下拉菜单）
...
::: details ⏱️ 调度器 (Scheduler) 3 大节奏表大拆解（图 2 下拉菜单）
