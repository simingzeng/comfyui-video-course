...
* **🎛️ `移位 (Shift)` 允许调多少？（避坑指南）**：
  * **正常有效取值区间**：👉 **`1.00 ~ 6.00`**（官方黄金推荐值：**`3.00`**，新手直接保持 3.00 即可）；
  * ❌ **严禁误填为 99.00**：如果误填为 99 等过大数值，时间曲线会发生严重畸变，AI 在第 1 瞬间耗尽算力后直接无法消除噪点，**画面会变成一片完全看不清的抽象色块或直接黑屏花屏**！
