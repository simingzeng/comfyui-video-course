...
在画布右侧，`K 采样器` 是全流程最核心的作画中枢，由 **主干 K 采样器面板（从上到下 6 大参数）** 与 **前置算法配件** 共同调控：

### 1. 物理核心：`K 采样器 (KSampler)` 6 大参数逐一拆解

#### 1️⃣ `种子 (seed)` 与 `生成后控制 (control)`
...
#### 3️⃣ `cfg`：画家的听话程度（引导尺度）
* 在极速流匹配/蒸馏模型中**必须死死锁定为 `1.0`**（完全依靠模型内在先验成画）；传统扩散模型通常为 6.0~8.0。

#### 4️⃣ `采样器名称 (sampler_name)`：画家的【运笔技法 / 削噪算法】
这是决定 AI 从纯噪点迷雾中削出清晰画面的数学求解路径，主流分为 4 大流派：
...
#### 5️⃣ `调度器 (scheduler)`：画家的【作画节奏表 / 去噪计划】
决定每一轮去噪到底擦除多少百分比的噪点，主流分为 3 种节奏：
...
#### 6️⃣ `降噪 (denoise)`：从零作画 vs 垫图重绘
* **`1.00`**：代表 100% 从零开始全新作画（文生图必须设为 1.00）；
* **`0.30 ~ 0.70`**：代表保留原图部分轮廓结构进行局部重绘（图生图）。
...
* **💡 既然刚才讲了步数只有 8 步，那 `移位 (Shift 3.00)` 到底在干嘛？**：
  ...
  * **`Shift 3.00` 的作用（精力平均切两份）**：就像一个【精力天平】，它把全部计算精力**平均切成了两半（50% : 50%）**：
    * 👉 **前一半精力 (50%)**：集中在前 1~3 步，光速锁定机械猫的主体铠甲骨架；
    * 👉 **后一半精力 (50%)**：留给后 4~8 步，全力打磨 8K 级金属反光与雨夜光影！
...
* **🔍 为什么在真机全景图中没看到独立的【保存图像】节点？**
  ...
  * **💾 自动存盘铁律**：无论画布上是否挂接独立保存节点，ComfyUI Desktop 每次运行生成的全部图片与视频，都会自动按日期持久化存放在【共享输出 (Shared output)】目录中：  
    👉 **官方默认路径**：`C:\Users\<你的用户名>\AppData\Local\Comfy-Desktop\ComfyUI-Shared\output\`  
    👉 **自定义迁移路径**：`D:\ComfyUI-Shared\output\`（若在设置中自定义过）  
    💡 *1秒直达快捷技巧*：在左上角点击【桌面端设置 ➔ 存储】，直接点击【共享输出】右侧的 **📁 文件夹图标** 即可瞬间秒开！
...
**👉 请点击下方进入下一课：[04. FLUX.2：文生图、图像编辑与多参考图](./02-flux-prompting.md)！**
