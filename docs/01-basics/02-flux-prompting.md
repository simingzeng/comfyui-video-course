...
在正式进入模型准备与节点搭建前，请先通晓以下 3 大核心基石：

### 1️⃣ 环境确认：继续沿用 MainEnv 实例，切勿新建！

::: details 🚨 动手前环境确认：继续沿用 MainEnv 实例，切勿新建！{open}

* **无需新建任何新实例**：
  进入阶段 1 后，很多同学会犹豫“要不要为 FLUX 单独新建一个实例”，答案是**绝对不要**！ComfyUI Desktop 官方桌面版的设计精髓是**“一套通用母舰（MainEnv）跑遍所有模型”**。
* **极速开启实操的两种状态（任选其一）**：
  * **状态 A（重新打开软件）**：启动 ComfyUI Desktop，在启动大厅的实例列表中，直接鼠标左键点击进入我们在阶段 0 建立的主力母舰 **`MainEnv`** 实例即可！
  * **状态 B（软件一直在开着）**：如果你的软件正停留在上一课的机械猫画布上，**无需退出软件**，直接点击 ComfyUI 界面右上角或左侧菜单的 **【清空画布】**（或新建一个工作流标签页），即可直接开始本课搭建。
* **模型与插件资产自动互通**：
  后续下载的全部 FLUX 模型文件均会自动保存在全局共享仓 `ComfyUI-Shared\models\` 中，在 `MainEnv` 实例内自由加载调度，与上一课的极速模型和后续的视频模型完全兼容互通！

:::

---

### 2️⃣ 必修通识：什么是 ComfyUI 插件？官方原生【管理扩展功能】一键实装

在开始下载模型前，花 2 分钟打通 ComfyUI 整个开源生态最核心的灵魂——**插件机制 (Custom Nodes)**：
...
#### 🖱️ 官方 Desktop 原生应用商店：纯鼠标 3 步一键安装任意插件（告别繁琐命令行！）
早期老旧教程往往要求读者在终端敲 `git clone` 或手动下载 zip 解压，而在 **ComfyUI Desktop 官方桌面版** 中，系统已经在顶部原生整合了功能强大的 **【自定义节点管理器】**，全程无需敲击任何命令：

1. **打开原生管理器**：在画布顶部右上角【▷ 运行】按钮的左侧，点击 **【管理扩展功能】** 按钮；
2. **搜索目标插件**：在弹出的【自定义节点管理器】窗口中，顶部搜索框输入目标插件名称：
   * 搜索并安装 **`ComfyUI-Manager`**：生态中老牌必备的综合管理插件；
   * （未来学习 Wan 2.2 视频或 ControlNet 时，同样直接在这里搜索对应名字即可安装！）；
3. **一键自动安装**：在搜索出的插件卡片右下角，直接点击亮蓝色的 **【📥 安装】** 按钮！系统就会全自动在后台将插件安装配置到 `custom_nodes` 中，完全免除手动搬运的繁琐！

::: details 🧰 原生管理器 2 大必知实用神级功能

* **功能 A：【缺失节点】自动修复（救命神器）**：
  * 在网上下载别人的工作流（.json 或图片拖入画布）时，如果画布上出现红色报错的“缺失节点”；
  * 只需打开【管理扩展功能】窗口，点击左侧菜单的 **【缺失节点】**，系统会自动扫描出当前工作流缺少的所有插件，点击一键全部补齐！
* **功能 B：【有可用更新】一键批量升级**：
  * 点击左侧 **【有可用更新】**，能快速查看已安装插件的最新版本并一键升级，时刻享受社区最新算法红利。

:::

::: details 💻 极客备用：无网络环境下的手动搬运法（Git / ZIP 离线导入）

如果你处于网络受限或离线环境，也可以通过传统手动方式放入：
* **Git 命令行法**：在 `custom_nodes` 目录下打开终端，执行 `git clone https://github.com/ltdrdata/ComfyUI-Manager.git`；
* **ZIP 手动解压法**：下载 ZIP 压缩包后解压，将文件夹重命名为对应插件名（如 `ComfyUI-Manager`），剪切粘贴进 `custom_nodes` 文件夹即可。

:::

#### 🔄 重启生效：见证插件激活！
* 插件安装完成后，点击提示框中的【重启】（或重启 ComfyUI Desktop 进入 `MainEnv` 实例）；
* 重启后，你刚才安装的所有全新节点和扩展功能就会 100% 自动装载入画布！

---

### 3️⃣ 双机硬件实测看板 (Benchmark)
